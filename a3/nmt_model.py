...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int], grader_params=None) -> Tuple[
        ...
        packed_X = pack_padded_sequence(X, lengths=source_lengths, enforce_sorted=False)
        ...
        last_hidden = torch.cat(torch.unbind(last_hidden, dim=0), dim=1)
        last_cell = torch.cat(torch.unbind(last_cell, dim=0), dim=1)
