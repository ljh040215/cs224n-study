...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 1. 随机截断
        # 长度在 [4, int(self.block_size*7/8)] 之间
        min_len = 4
        ...
        # 对文档进行截断。如果文档长度小于随机选择的长度，则取整个文档
        # 实际长度可能会小于 trunc_len，更新 trunc_len 为实际长度
        sample = document[:trunc_len]
        trunc_len = len(sample)

        # 边缘情况处理：如果样本过短（<4），虽然题目假设了范围，但为了稳健性，
        # 我们可以简单地重新取整个文档或者不做掩码（这里按题目逻辑继续，依赖数据质量）

        # 2. 构造掩码内容
        # 掩码长度应该是截断长度的 1/4 左右。
        # 我们从 [1, trunc_len // 2] 中均匀采样，期望值约为 trunc_len / 4 + 0.5
        # 使用 max(1, ...) 防止 trunc_len 很小时范围无效
        mask_len = random.randint(1, max(1, int(trunc_len / 2)))

        # 随机选择掩码的起始位置
        # 有效范围是 [0, trunc_len - mask_len]
        mask_start = random.randint(0, trunc_len - mask_len)

        prefix = sample[:mask_start]
        masked_content = sample[mask_start: mask_start + mask_len]
        suffix = sample[mask_start + mask_len:]

        # 3. 重排字符串
        # 格式: [prefix] MASK [suffix] MASK [masked_content] [pads]
        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content

        # 填充 (Padding)
        # 目标总长度为 self.block_size + 1
        target_len = self.block_size + 1
        pad_len = target_len - len(masked_string)
        if pad_len > 0:
            masked_string += self.PAD_CHAR * pad_len

        # 4. 构造输入输出对 (x, y)
        # x 是 masked_string 去掉最后一个字符
        # y 是 masked_string 去掉第一个字符
